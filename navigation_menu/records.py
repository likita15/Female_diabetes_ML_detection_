...
def delete():
        ...
        with st.form(key="delete", clear_on_submit=True):
            ...
            if delete_button:
                ...
                sql2 = "delete from ids where id = %s"
...
def show():
    ...
    with tab5:
        ...
        option = st.selectbox(
            ...
            ("Add New Record", "Update Record", "View Record", "Delete Record"),
            key="chooseRecord"
        )
        ...
        if option == "Add New Record":
            ...
            option_inside = st.selectbox("Select ids for deletion of records", list(record_df["id"]),key='labdeletion')
            patient_id = option_inside

            with st.form(key="delete_record", clear_on_submit=True):
                st.title("Delete Test Results Records")


                submit_button = st.form_submit_button("Delete")

            if submit_button:
                sql_delete = "delete from lab_data WHERE id = %s"
                val_delete = (patient_id,)
    ...
    with tab6:
        ...
        try:
            ...
            MailTest_report.ToMail(option_inside)

        except Exception:
            st.subheader("No records found")
